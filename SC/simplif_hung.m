...
for k = 1:dim
    minim = min(mat(k,:));
    if minim ~= inf   %encontrou números sem ser inf na linha k
        mlin(k,:) = (mat(k,:) == minim); %coloca 1s na linha k nas posições 
                                       %com valores igual ao mínimo
    end
end
...
for k = 1:dim
    minim = min(mat(:,k));
    if minim ~= inf   %encontrou números sem ser inf na coluna k
        mcol(:,k) = (mat(:,k) == minim); %coloca 1s na coluna k nas posições 
                                       %com valores igual ao mínimo
    end
end
