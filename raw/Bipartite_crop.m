...
images = strtrim(cellstr( num2str((1:noOfFrames)','alexis%d') )); 
...
%          histograms=reshape(histograms,12,5,41);
...
imagesc(CM);
colorbar; 
title(['CM with', num2str(noPersons) 'people, R=Perimeter/2, NN Classifier Accuracy:', num2str(100*Accuracy),'using Jacinto code  for SC'])
noPersons=size(y,1)/2
